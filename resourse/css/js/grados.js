$(document).ready(function() {
            ...
            var datatable = $('#dataTable').DataTable({
                ajax: "materia.php?m=ajaxtabla_grado",
                columns: [
                    {data:"0"},
                    {data:"1"},
                    {data:"2"},
			...
			$("#dataTable").on('click','#view',function(){
				 ...
				 $('.modal2').load('materia.php?m=view1_grados&id='+col1,function(){
			...
			$("#dataTable").on('click','#edit',function(){
				...
				$.get('materia.php?m=edit1_grados&id='+col1,function(data){
					$("#InputNombrecss_u").attr("value",data['id_grado']);
					$("#InputPuntajecss_u").attr("value",data['nombre_grado']);
					$("#InputDescripcioncss_u").attr("value",data['cant_est']);
			...
			$("#dataTable").on('click','#del',function(){
				...
				if (seleccion)
				{
					$.get('materia.php?m=delete1_grados&id='+col1,function(data){
			...
			$("#btm_update").on('click',function(){
				$.ajax({
					  url:"materia.php?m=update1_grados",
					  type: "POST",
					  data: {
			...
			$('#btm_guardar').on('click',function(){
				$.ajax({
					  url:"materia.php?m=insert1_grados",
					  type: "POST",
					  data: {
						InputGradocss: $("#InputGradocss").val(),
						InputCantidadncss: $("#InputCantidadncss").val()
					  }
				})
				.done(function( data, xhr ) {
					//console.log(data["valor"]);
			...
			/*$.ajax({
				type: "GET",
				url: 'materia.php?m=get_tipo_cliente', 
				dataType: "json",
