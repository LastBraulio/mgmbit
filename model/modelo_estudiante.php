...
Class Modelo_estudiante extends Conexiones{
	public $tabla = "countries";
	
	public function mostrar_ajax2()
	{
		//$sentencia = "select * from countries";
		$sentencia = "Select t1.cedula, t1.nombre, t1.telefono, t1.email, t2.nombre_perfil from usuarios t1 left join usu_usuario_perfil t2 on t1.cedula = t2.cedula left join perfiles t3 on t3.descripcion = t2.nombre_perfil where t3.descripcion='Estudiante' AND t1.cedula NOT IN (select cedula_stu from asignar_estudiantes_materia);";
		//var_dump($this->pdo);
		//exit;
	...
	{
		//$sentencia = "select * from countries";
		$sentencia = "Select t1.cedula, t1.nombre, t1.telefono, t1.email, t2.nombre_perfil from usuarios t1 left join usu_usuario_perfil t2 on t1.cedula = t2.cedula left join perfiles t3 on t3.descripcion = t2.nombre_perfil where t3.descripcion='Estudiante';";
		//var_dump($this->pdo);
		//exit;
		$resultado = $this->pdo->prepare($sentencia);
		$html="";
	...
	public function mostrar_ajax_3($ced)
	{
		$sentencia = "select t1.cedula_stu, t2.nombre, t3.nom_materia, t4.nombre_grado, t5.descripcion,t1.ced_profesor from (select * from view_asignarprofe_estudiante) t1, usuarios t2, materia t3, grados t4, jornada t5 where t1.cedula_stu = t2.cedula and t1.id_materia = t3.id_materia and t1.id_grado = t4.id_grado and t1.id_jornada = t5.id_jornada and t1.ced_profesor = '$ced' ";
		//echo $sentencia; exit;

		//$sentencia = "select t1.cedula_stu, t2.nombre, t3.nom_materia, t4.nombre_grado, t5.descripcion from asignar_estudiantes_materia t1, usuarios t2, materia t3, grados t4, jornada t5 where t1.cedula_stu = t2.cedula  and t1.id_materia = t3.id_materia and t1.id_grado = t4.id_grado and t1.id_jornada = t5.id_jornada ";
	...
	{
		$sql = "INSERT INTO asignar_estudiantes_materia (cedula_stu,id_materia,id_grado,id_jornada) VALUES (:cedula_stu, :id_materia, :id_grado, :id_jornada )";
		try{
			//var_dump($this->pdo); exit;
			//print_r($data);
			$stmt= $this->pdo->prepare($sql);
